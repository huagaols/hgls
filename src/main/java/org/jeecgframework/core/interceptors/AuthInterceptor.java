	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object object) throws Exception {
		...
		if (excludeUrls.contains(requestPath)) {
			...
			if (client != null && client.getUser()!=null ) {
				...
				//update-begin--Author:zhoujf  Date:20150804 for：流程节点id获取
				String processnodeId = request.getParameter("processnodeId");
				//update-end--Author:zhoujf  Date:20150804 for：流程节点id获取
				...
				if(!oConvertUtils.isEmpty(functionId)){
					...
					//update-begin--Author:zhoujf  Date:20150804 for：流程节点页面控件权限
					if(!oConvertUtils.isEmpty(processnodeId)){
						Set<String> nodeOperationCodes = activitiService.getNodeOperaCodesByNodeIdAndFunctionId(processnodeId, functionId);
						operationCodes.addAll(nodeOperationCodes);
					}
					//update-end--Author:zhoujf  Date:20150804 for：流程节点页面控件权限
...
//---author:jg_xugj----end-----date:20151210--------for：#781  【oracle兼容】兼容问题fun.operation!='' 在oracle 数据下不正确
					    ...
					    //update-begin--Author:zhoujf  Date:20150804 for：流程节点页面按钮权限
						if(!oConvertUtils.isEmpty(processnodeId)){
							Set<String> nodeOperationCodes = activitiService.getNodeOperaCodesByNodeIdAndFunctionId(processnodeId, functionId);
						...
						//update-end--Author:zhoujf  Date:20150804 for：流程节点页面按钮权限
					}
					...
					//update-begin--Author:zhoujf  Date:20150804 for：流程节点数据规则权限获取
					if(!oConvertUtils.isEmpty(processnodeId)){
						Set<String> nodeDataruleCodes = activitiService.getOperationCodesByNodeIdAndruleDataId(processnodeId, functionId);
						dataruleCodes.addAll(nodeDataruleCodes);
					}
					//update-end--Author:zhoujf  Date:20150804 for：流程节点数据规则权限获取
